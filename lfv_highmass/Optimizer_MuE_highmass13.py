...
class Optimizer_MuE_highmass13(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        self.channel='EMu'
        super(Optimizer_MuE_highmass13, self).__init__(tree, outfile, **kwargs)
...
#                elif jn==2:
 #                   category=2 if self.shifted_vbfMass<550 else 3
    ...
    #                if self.my_muon.Pt()<70:continue
                    ...
                    passed_cuts=optimizer.compute_regions_0jet(mPt=self.my_muon.Pt(),ePt=self.my_elec.Pt(),dphiEMu=deltaPhi(self.my_elec.Phi(),self.my_muon.Phi()),eDPhiToPfMet=abs(self.shifted_eDPhiToPfMet),mDPhiToPfMet=abs(self.shifted_mDPhiToPfMet),mMtToPfMet=self.shifted_mMtToPfMet,eMtToPfMet=self.shifted_eMtToPfMet)

                if category == 1 :
                    if abs(self.shifted_eDPhiToPfMet)>0.3:continue
 #                   if self.my_muon.Pt()<70:continue
                    if deltaPhi(self.my_elec.Phi(),self.my_muon.Phi())<2.2:continue
                    passed_cuts=optimizer.compute_regions_1jet(mPt=self.my_muon.Pt(),ePt=self.my_elec.Pt(),dphiEMu=deltaPhi(self.my_elec.Phi(),self.my_muon.Phi()),eDPhiToPfMet=abs(self.shifted_eDPhiToPfMet),mDPhiToPfMet=abs(self.shifted_mDPhiToPfMet),mMtToPfMet=self.shifted_mMtToPfMet,eMtToPfMet=self.shifted_eMtToPfMet)
